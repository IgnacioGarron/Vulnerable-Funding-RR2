...
for (h in h_horizon){
  ...
  data_reg <- data %>% group_by(country) %>%
    ...
    mutate(stock_h=lead(stock_z,h),
           ...
           gdp_h=lead(gdp_z,h),
           credit_z_l2=lag(credit_z,2),
           credit_z_l3=lag(credit_z,3),
           credit_z_l4=lag(credit_z,4))
  ...
  for (country_name in credit_countries){ # acá empieza loop para país CAMBIAR PARA GAR E EAR
    ...
    data_model<-data_reg %>% 
      group_by(country) %>% 
      filter(complete.cases(credit_h,credit_z,NFCI_z,f_global_z,
                            credit_z_l2,credit_z_l3,credit_z_l4),country==country_name) 
    ...
    if (h==0){
      X.train1<-as.matrix(cbind(rep(1,length(Y.train[,1])),data_model[,c("NFCI_z","Real_z","credit_f_z","f_global_z",
                                                                         "credit_z_l2","credit_z_l3","credit_z_l4")]))
...
save(coef_b1,file ="../Data/M1_credit_lags_coef_b1.RData")
save(coef_b2,file ="../Data/M1_credit_lags_coef_b2.RData")
save(coef_b3,file ="../Data/M1_credit_lags_b3.RData")
save(coef_b4,file ="../Data/M1_credit_lags_b4.RData")
save(coef_b5,file ="../Data/M1_credit_lags_b5.RData")


save(sig_b1,file ="../Data/M1_credit_lags_sig_b1.RData")
save(sig_b2,file ="../Data/M1_credit_lags_sig_b2.RData")
save(sig_b3,file ="../Data/M1_credit_lags_sig_b3.RData")
save(sig_b4,file ="../Data/M1_credit_lags_sig_b4.RData")
save(sig_b5,file ="../Data/M1_credit_lags_sig_b5.RData")


save(TL1,file ="../Data/M1_credit_lags_TL.RData")
save(TL2,file ="../Data/M2_credit_lags_TL.RData")
save(Pred_list,file ="../Data/M1_credit_lags_Pred.RData")
